...
BetterHolonomicDriveController::BetterHolonomicDriveController(
  ...
  m_xController.SetTolerance(m_poseTolerance.Translation().X().to<double>(), 0.5);
  m_yController.SetTolerance(m_poseTolerance.Translation().Y().to<double>(), 0.5);
  m_thetaController.SetTolerance(m_poseTolerance.Rotation().Radians(), 0.5_rad_per_s);
  m_thetaController.EnableContinuousInput(0_deg, 360.0_deg);
}

bool BetterHolonomicDriveController::AtReference() const {
...
void BetterHolonomicDriveController::SetTolerance(const Pose2d& tolerance) {
...
ChassisSpeeds BetterHolonomicDriveController::Calculate(
...
void BetterHolonomicDriveController::SetEnabled(bool enabled) {
  m_enabled = enabled;
}